"""Script templates executed by PanGen's bundled Python.

Keep these templates compatible with old Python interpreters. They are written
to each run directory and executed by the PanGen binary, while the main
term_agent package continues to run on modern Python.
"""
...
def write_pangen_scripts(run_dir: Path) -> None:
    (run_dir / "direct_pframe.py").write_text(DIRECT_PFRAME, encoding="utf-8")
    (run_dir / "direct_task.py").write_text(DIRECT_TASK, encoding="utf-8")
